...
TByte Utf8::FirstByte()
{
    ...
    TByte byte = iBuffer.Uint8At(iIndex++); // get first byte
...
{
    if (End()) { THROW(InvalidString); }
    TByte byte = iBuffer.Uint8At(iIndex++); // get next byte
...
void String::Replace(const Brx& aBuffer)
{
    ...
    else if (iOriginalEncoding == eLatin1)
    {
        ...
        for (TUint i = 0; i < aBuffer.Bytes(); i++) {
            iBuffer.AppendUint32(static_cast<TUint32>(aBuffer.At(i)));
        }
    }
    else // utf8 (default)
    {
        ...
        while (!utf8.End())
        {
            iBuffer.AppendUint32(static_cast<TUint32>(utf8.Next()));
        }
    }
}
...
{
    return(iBuffer.BeUint32At(aCharIndex*4));
}
...
void String::ToUtf8(Bwx& aBuffer)
{
    ...
    try {
        ...
        LOG(kSysLib, "String::ToUtf8 --> BufferOverflow\n");
    }
}
...
void String::ReorderRightToLeft() {
    ...
    Bwd typeBuffer(Brx::Empty());
    typeBuffer.Grow(Chars());
    typeBuffer.SetBytes(0);
    for (TUint i = 0; i < Chars(); i++) {
        typeBuffer.AppendUint8(BidiCharType(At(i)));
    }
    ...
    for (TInt i = ((TInt)bufferReorder.Bytes() - 1);  i >= 0; i--) {
        typeBuffer.AppendUint8(bufferReorder[i]);
    }
    ...
    for (TUint i = 0; i < typeBuffer.Bytes(); i++) {
        bufferReorder.AppendUint32(At(typeBuffer[i]));
    }
